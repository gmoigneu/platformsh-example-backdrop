...
if (empty($_ENV['PLATFORM_RELATIONSHIPS'])){
  ...
  // Setup platform.sh database settings
  $relationships = json_decode(base64_decode($_ENV['PLATFORM_RELATIONSHIPS']), TRUE);
  $database = 'mysql://'.$relationships['database'][0]['username'].':'.$relationships['database'][0]['password'].'@'.$relationships['database'][0]['host'].'/'.$relationships['database'][0]['path'];
  $database_prefix = '';
}
...
if (isset($_ENV['PLATFORM_ROUTES'])) {
  ...
  $base_url = $site_scheme . '://' . $site_host; // NO trailing slash!
}
